...
static void http_rest_with_url(void)
{
    ...
    // esp_http_client_set_header(client, "Authorization", "Bearer ZvBpR0XoFehfP5D68QZfHL4N5hchUrHjXzaFVGjW");
    // Valle Token
    // esp_http_client_set_header(client, "Authorization", "Bearer Nto7vv6I7Qw0wgS6kj0yCF0iXlVZMkpddiofrjFv");
    // Salon Room Token
    // esp_http_client_set_header(client, "Authorization", "Bearer aUboOdWtczFGEa6FDFmillamqvuuAPhEPIoJNanF");
...
void onboardLed(void *pvParameters)
{
    // TODO: Find how to pass parameters to freertos task.
    led_strip_t strip = {
        .type = LED_TYPE,
    ...
    ESP_ERROR_CHECK(led_strip_fill(&strip, 0, strip.length, colors[ledStatus]));
...
void app_main(void)
{
    ...
    // Init led and red status, obtaining wifi.
    led_strip_install();
    ledStatus = 1;
    xTaskCreate(onboardLed, "onboardLed", 8192, NULL, 5, NULL);
    ...
    // Green status, we have wifi, sending signal
    ledStatus = 2;
    xTaskCreate(onboardLed, "onboardLed", 8192, NULL, 5, NULL);
